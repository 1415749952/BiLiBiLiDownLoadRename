...
public class FileToolkit
{
   // private static final Log log = LogFactory.getLog(FileToolkit.class);
     ...
     * @param object 文件类型过滤器
     * @return 当前目录下面的所以该类型的文件
     */
    public static List<File> getFileFromDirectory(File directory, FileFilter object)//遍历整个目录全部文件
    {
        //System.out.println(file);
        List<File> fileList = new ArrayList<File>();


        File[] listFiles = directory.listFiles(object);
        for (File file2 : listFiles)
        {
            //System.out.println(file2);
            if (file2.isDirectory())
            {
                fileList.addAll(getFileFromDirectory(file2,object));
            }
            else {
                //System.out.println(file2);
                fileList.add(file2);
